class_name TileHighlighter
extends Node

@export var allowed_tile_color: Color
@export var unallowed_tile_color: Color
@export var enabled: bool = true : set = _set_enabled
@export var play_area: PlayArea
@export var highlight_layer: TileMapLayer
@onready var highlight: TileMapLayer = $"../Highlight"

@onready var source_id := highlight.tile_set.get_source_id(0)

var last_selected_tile := Vector2i(-1, -1)

func _process(_delta) -> void:
	if not enabled:
		return
		
	var selected_tile := play_area.get_hovered_tile()
	if selected_tile == last_selected_tile:
		return

	if not play_area.is_tile_in_bounds(selected_tile):
		highlight_layer.clear()
		last_selected_tile = selected_tile
		return
		
	_update_tile(selected_tile)
	last_selected_tile = selected_tile
	
func _set_enabled(new_value: bool) -> void:
	enabled = new_value
	
	if not enabled and play_area:
		highlight_layer.clear()
		
func _update_tile(selected_tile: Vector2i) -> void:
	var atlas_tile := play_area.play_area_floor.get_cell_atlas_coords(selected_tile)

	highlight_layer.clear()
	highlight_layer.set_cell(selected_tile, source_id, atlas_tile)
